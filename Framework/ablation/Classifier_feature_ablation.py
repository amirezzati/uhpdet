...
def main(args: argparse.Namespace) -> None:
    ...
    # Determine ALL numeric feature columns (excluding label and ID columns)
    numeric_cols = merged_df.select_dtypes(include=[np.number]).columns.tolist()
    
    # Filter out non-feature columns like 'id', 'baseline', 'truth' (if string), and the label itself
    exclude_cols = {args.label_col, "id", "baseline"}
    # Also exclude string columns that might be in numeric_cols list if they weren't filtered properly (though select_dtypes handles this)
    # We also want to exclude 'truth' if it exists, though it's usually string.
    if 'truth' in merged_df.columns:
        exclude_cols.add('truth')
        
    all_feature_cols = [c for c in numeric_cols if c not in exclude_cols]
    
    if not all_feature_cols:
        logger.error("No numeric feature columns found to perform ablation study.")
        return

    logger.info("Total features found for ablation: %d", len(all_feature_cols))
    
    # Drop duplicates once before starting the loop
    ...
    logger.info("="*80)
    logger.info("Running BASELINE experiment with ALL features")
    logger.info("="*80)
    baseline_output_dir = os.path.join(run_output_dir, "baseline_all_features")
    ensure_dir_exists(baseline_output_dir)
    
    summary_df = build_and_evaluate(
        unique_df, 
        args.label_col, 
        all_feature_cols, 
    ...
    summary_df['experiment'] = 'baseline_all_features'
    ablation_results.append(summary_df)
    
    # 2. Run Ablation: Delete one feature per run
    for feature_to_delete in all_feature_cols:
        logger.info("="*80)
        logger.info(f"Running Ablation: WITHOUT feature '{feature_to_delete}'")
        logger.info("="*80)
        
        # Create feature list excluding the current one
        current_features = [f for f in all_feature_cols if f != feature_to_delete]
        
        # Define output directory for this specific ablation run
        # Format: .../output_dir/run_name/without_[feature_name]
        safe_feature_name = sanitize_filename(feature_to_delete)
        ablation_output_dir = os.path.join(run_output_dir, f"without_{safe_feature_name}")
        ensure_dir_exists(ablation_output_dir)
        
        try:
            summary_df_iter = build_and_evaluate(
                unique_df, 
                args.label_col, 
                current_features, 
            ...
            summary_df_iter['experiment'] = f'without_{safe_feature_name}'
            summary_df_iter['removed_feature'] = feature_to_delete
            ablation_results.append(summary_df_iter)
        except Exception as e:
            logger.error(f"Failed to run ablation for feature {feature_to_delete}: {e}")

    # --- Aggregate and Save Ablation Summary ---
    if ablation_results:
        # Concatenate all summary dataframes
        final_ablation_df = pd.concat(ablation_results)
        
        # Reset index to handle model names properly
        final_ablation_df.reset_index(inplace=True)
        final_ablation_df.rename(columns={'index': 'model_name'}, inplace=True)
        
        # Save the master ablation summary
        master_summary_path = os.path.join(run_output_dir, "ablation_study_master_summary.csv")
        final_ablation_df.to_csv(master_summary_path, index=False)
        logger.info("Saved master ablation summary to %s", master_summary_path)
