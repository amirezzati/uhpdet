...
# ---------- I/O helpers ----------
def load_and_merge_csv(file1: str, file2: str) -> pd.DataFrame:
...
def get_path(model: str, benchmark_base: str, q_type: str) -> str:
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir) # Adjust based on your folder depth
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-dir", default="../Results/Classifier/", help="Output directory.")
...
#     p.add_argument("--results-dir", default="/home/user01/haldet/Framework/results", help="Directory where results_full_* files live (default shown).")
#     p.add_argument("--label-col", default="hal_label", help="Name of label/target column in features CSV (default: hal_label).")
#     p.add_argument("--output-dir", default="./results_output", help="Directory to write outputs (plots, models, summaries).")
...
"""
Example usage (run from the Framework/ directory):

Automatic path derivation (reads from ../Results/Features/<model>/<benchmark>_<type>/):
    python classifier.py --model blip --benchmark AMBER --type both

Manual file paths:
    python classifier.py --file1 ../Results/Features/blip/AMBER_yes_questions/data_features.csv \\
        --file2 ../Results/Features/blip/AMBER_no_questions/data_features.csv \\
        --model blip --benchmark amber
"""